 ...
 */
const stream = (elastic, query, timestampKey, options={ timescale: 1 }) => {
	let timeOffset;
	let startDate;
	...
	;(async () => {
		for await (const result of scrollSearch(elastic, query)) {
			...
			for (entity of result.hits.hits) {
				// const scaledCurrentTime = Date.now()+((Date.now()-startDate)*options.timescale)
				...
				// process.stdout.write(`Current Time: ${new Date(scaledCurrentTime).toISOString()} <- ${options.timescale}x ${new Date().toISOString()}\nEntity  Time: ${new Date(offsetEntityTime).toISOString()} <-    ${new Date(entity._source[timestamp]).toISOString()}\n\n`)
